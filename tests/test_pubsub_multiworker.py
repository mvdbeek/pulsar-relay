...
pytestmark = pytest.mark.skipif(
    not os.getenv("VALKEY_INTEGRATION_TEST"),
    reason="VALKEY_INTEGRATION_TEST environment variable not set"
)
...
class TestMultiWorkerPubSub:
    ...
    async def test_multiple_messages_broadcast_correctly(self, real_server):
        ...
        for client_id, messages in enumerate(received_messages):
            ...
            assert len(received_ids) >= num_messages * 0.8, \
                f"Client {client_id} only received {len(received_ids)}/{num_messages} messages"

        print(f"✓ All clients received all messages via pub/sub")
    ...
    async def test_pubsub_coordinator_with_long_polling(self, real_server):
        ...
        poll_tasks = [
            asyncio.create_task(poll_client(i))
            for i in range(3)
        ]
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={
                    "topic": topic,
                    "payload": {"test": "polling-broadcast"}
                },
                headers={"Authorization": f"Bearer {token}"}
            )
            assert response.status_code == 201
        ...
        assert received_count >= 2, \
            f"Only {received_count}/3 poll clients received the message via pub/sub"

        print(f"✓ Long-polling clients received messages via pub/sub")
